...
class spin_lock_TAS{
	...
	public:
	spin_lock_TAS() {}
	~spin_lock_TAS() {assert(m_spin.load(memory_order_acquire)==0);}
	string name(){return "Spin lock TAS time                  |";}
	void lock(){
...
class yield_spin_lock_TAS{
atomic<unsigned int> m_spin=0;
	public:
	yield_spin_lock_TAS() {}
	~yield_spin_lock_TAS() {assert(m_spin.load(memory_order_acquire)==0);}
	string name(){return "Spin lock TAS with yield time       | ";}
	void lock(){
...
class exp_bo_spin_lock_TAS{
atomic<unsigned int> m_spin=0;
	public:
	exp_bo_spin_lock_TAS(){}
	~exp_bo_spin_lock_TAS() {assert(m_spin.load(memory_order_acquire)==0);}
	string name(){return "Spin lock TAS with exp back off     | ";}
	void lock(){
		unsigned int expected=0;
		size_t max_power = 15;
...
class yield_exp_bo_spin_lock_TAS{
atomic<unsigned int> m_spin=0;
	public:
	yield_exp_bo_spin_lock_TAS() {}
	~yield_exp_bo_spin_lock_TAS() {assert(m_spin.load(memory_order_acquire)==0);}
	string name(){return "Spin lock TAS with yield and exp bo | ";}
	void lock(){
		unsigned int expected=0;
		size_t max_power = 15;
		//double max_time = 0.01*pow(2, max_power);
...
spin_lock_TAS TAS;
exp_bo_spin_lock_TAS TAS_exp;
yield_spin_lock_TAS TAS_yield;
yield_exp_bo_spin_lock_TAS TAS_yield_exp;
my_mutex mtx;
template <typename T>
...
int main(){
	...
	Benchmark<spin_lock_TAS>(TAS, i, count);
	Benchmark<exp_bo_spin_lock_TAS>(TAS_exp, i, count);
	Benchmark<yield_spin_lock_TAS>(TAS_yield, i, count);
	Benchmark<yield_exp_bo_spin_lock_TAS>(TAS_yield_exp, i, count);
	}
	return 0;
}
